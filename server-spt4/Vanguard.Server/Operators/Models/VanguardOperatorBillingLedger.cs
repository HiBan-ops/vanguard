// Responsibility: Defines data/state contracts used by the Operator persistence/domain models, centered on Operator Billing Ledger.
...
public sealed record VanguardOperatorBillingLedger(
    IReadOnlyList<VanguardOperatorBillingInvoice> Invoices,
    IReadOnlyList<VanguardOperatorBillingNotification> Notifications,
    int SchemaVersion,
    DateTimeOffset UpdatedAtUtc,
    VanguardOperatorBillingSettlementReceipt? PendingSettlement = null)
{
    public static VanguardOperatorBillingLedger Empty(DateTimeOffset now) => new(
        Array.Empty<VanguardOperatorBillingInvoice>(),
        Array.Empty<VanguardOperatorBillingNotification>(),
