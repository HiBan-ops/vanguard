...
internal static class VanguardHardReturnMovementExecutor
{
    ...
    // Vanguard: legacy reflection GoToPoint bridge retained only for source audit; HardReturn backend is BigBrain GoToSomePoint.
    private static bool TryCommandEftHardReturn(BotOwner botOwner, Vector3 target, bool sprint, out string result)
    {
        Vector3 currentPosition = ResolveBotPosition(botOwner);
        if (!TryValidateCompletePath(currentPosition, target, out var preCommandPath))
        {
            result = "command_blocked_path_invalid:" + preCommandPath;
            return false;
        }

        var diagnostics = new List<string>(10);
        diagnostics.Add("path=" + preCommandPath);
        object? mover = VanguardOperatorRuntimeAuditReflection.GetPropertyOrField(botOwner, "Mover", "BotMover");
        diagnostics.Add("sprint=" + Bool(TryInvoke(mover, "Sprint", sprint)));
        diagnostics.Add("targetSpeed=" + Bool(TryInvoke(mover, "SetTargetMoveSpeed", sprint ? 1.25f : 1.0f) || TryInvoke(botOwner, "SetTargetMoveSpeed", sprint ? 1.25f : 1.0f)));
        bool sawCommandApi = false;
        if (TryCommandMover(mover, target, "eftMover", diagnostics, ref sawCommandApi)
            || TryCommandBotOwner(botOwner, target, "botOwner", diagnostics, ref sawCommandApi))
        {
            result = "movement_commanded_path_complete:" + string.Join("|", diagnostics);
            return true;
        }

        result = (sawCommandApi ? "movement_command_unreliable" : "movement_command_unavailable") + ":" + string.Join("|", diagnostics);
        return false;
    }

    // Legacy reflection helper retained only for audit. Not called by Vanguard HardReturn.
    private static bool TryCommandMover(object? mover, Vector3 target, string label, List<string> diagnostics, ref bool sawCommandApi)
    {
        if (mover == null)
        {
        ...
        string type = VanguardOperatorRuntimeAuditReflection.TypeName(mover);
        if (TryInvokeMoveCommand(mover, "CurrentStateGoToPoint", out var current, target, false, 0.75f, false, true, false, true))
        {
            sawCommandApi = true;
            diagnostics.Add(label + ":" + type + ":CurrentStateGoToPoint=" + current);
            if (IsPositiveMoveResult(current)) return true;
        }

        if (TryInvokeMoveCommand(mover, "GoToPoint", out var goForced, target, false, 0.75f, false, true, false, true))
        {
            sawCommandApi = true;
            diagnostics.Add(label + ":" + type + ":GoToPoint7=" + goForced);
            if (IsPositiveMoveResult(goForced)) return true;
        }

        if (TryInvokeMoveCommand(mover, "GoToPoint", out var goBasic, target, false, 0.75f, false, true))
        {
            sawCommandApi = true;
            diagnostics.Add(label + ":" + type + ":GoToPoint5=" + goBasic);
            if (IsPositiveMoveResult(goBasic)) return true;
        }

        diagnostics.Add(label + ":" + type + ":GoToPointNoWay_skipped_not_reliable");
        return false;
    }

    // Legacy reflection helper retained only for audit. Not called by Vanguard HardReturn.
    private static bool TryCommandBotOwner(BotOwner botOwner, Vector3 target, string label, List<string> diagnostics, ref bool sawCommandApi)
    {
        string type = VanguardOperatorRuntimeAuditReflection.TypeName(botOwner);
        if (TryInvokeMoveCommand(botOwner, "GoToPoint", out var goSingle, target))
        {
            sawCommandApi = true;
            diagnostics.Add(label + ":" + type + ":GoToPoint1=" + goSingle);
            if (IsPositiveMoveResult(goSingle)) return true;
        }

        if (TryInvokeMoveCommand(botOwner, "GoToPoint", out var goBasic, target, false, 0.75f, false, true))
        {
            sawCommandApi = true;
            diagnostics.Add(label + ":" + type + ":GoToPoint5=" + goBasic);
            if (IsPositiveMoveResult(goBasic)) return true;
        ...
        if (TryInvokeMoveCommand(botOwner, "GoToPoint", out var goForced, target, false, 0.75f, false, true, false, true))
        {
            sawCommandApi = true;
            diagnostics.Add(label + ":" + type + ":GoToPoint7=" + goForced);
            if (IsPositiveMoveResult(goForced)) return true;
        ...
        diagnostics.Add(label + ":" + type + ":no_reliable_botowner_go_command");
        return false;
    }
    ...
    private static string FormatMoveReturn(object? returnValue)
    {
        ...
        return returnValue.GetType().Name + ":" + Safe(returnValue.ToString());
    }

    private static bool IsPositiveMoveResult(string result)
    {
        return result.Contains("PathComplete", StringComparison.OrdinalIgnoreCase)
            && !result.Contains("PathInvalid", StringComparison.OrdinalIgnoreCase)
            && !result.Contains("PathPartial", StringComparison.OrdinalIgnoreCase)
            && !result.Contains("NoWay", StringComparison.OrdinalIgnoreCase)
            && !result.Contains("void:", StringComparison.OrdinalIgnoreCase)
            && !result.Contains("bool:false", StringComparison.OrdinalIgnoreCase)
